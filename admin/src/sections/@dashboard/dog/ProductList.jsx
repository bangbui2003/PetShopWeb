                  ...
                  <Table>
                    ...
                    <TableBody>
                      {filteredDogs.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                        const { id, DogName, Images, DogSpecies, Price, created_at,
                          updated_at, IsInStock, deleted_at } = row;
                        const selectedDog = selected.indexOf(DogName) !== -1;
                        return (
                          <TableRow hover key={id} tabIndex={-1} role="checkbox" selected={selectedDog}>
                            ...
                            <TableCell align="left">{convertDogSpeciesToName(DogSpecies)}</TableCell>
                            <TableCell align="left">{Price > 0 ? `${valueLabelFormat(Price)}` : "0"}</TableCell>

                            <TableCell align="left">{fDateTime(created_at)}</TableCell>

                            <TableCell align="left">{updated_at ? `${fDateTime(updated_at)}` : "Chưa chỉnh sửa"}</TableCell>

                            <TableCell align="left">
                              <Label color={deleted_at ? "error" : !IsInStock ? "warning" : "success"}>
                                {
                                  deleted_at ? "Bị xóa" : "Có sẵn"
                                } / {
