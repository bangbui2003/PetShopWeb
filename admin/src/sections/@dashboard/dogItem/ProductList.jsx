...
export default function ProductList() {
  ...
  const getAll = async () => {
    try {
      const { response, err } = await itemApi.getAllItem();
      if (response) {
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {filteredItems.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)?.map((row) => {
                        const { ItemName, Price, Category, Quantity, Images, created_at,
                          updated_at, deleted_at, IsInStock, id } = row;
                        const selectedDogItem = selected.indexOf(ItemName) !== -1;
                        return (
                          <TableRow hover key={id} tabIndex={-1} role="checkbox" selected={selectedDogItem}>
